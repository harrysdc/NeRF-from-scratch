...
class Model(torch.nn.Module):
    """
    Model contains
      1) Implicit functions defining the scene
      2) Ray points sampler
      3) Renderer
    """
    def __init__(self, cfg):
        ...
        self.implicit_fn = volume_dict[cfg.implicit_function.type](cfg.implicit_function)
...
def render_images(
    ...
    for cam_idx, camera in enumerate(cameras):
        ...
        if cam_idx == 0 and file_prefix == '':
            ray_bundle_img = vis_rays(ray_bundle, image_size)
            plt.imsave(f'images/rays.png', ray_bundle_img)
        ...
        # ([1, H*W*num_pts_per_ray, 3])

        # TODO (1.4): Visualize sample sample_points as point cloud
        if cam_idx == 0 and file_prefix == '':
            rendered_img = render_points(f'{file_prefix}_{cam_idx}_points.png', sample_points, image_size)
        ...
        if cam_idx == 2 and file_prefix == '':
            ...
            plt.imsave("images/depth.png", depth)
