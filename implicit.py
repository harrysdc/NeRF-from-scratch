...
class MLPWithInputSkips(torch.nn.Module):
    ...
    Additionally, for a set of predefined layers `input_skips`, the forward pass sappends a skip tensor `z`
    """
    def __init__(
        self,
        n_layers: int,
...
class NeuralRadianceField(torch.nn.Module):
    def __init__(self, cfg):
        ...
        # harmonic embedding of 3D point locations
        self.harmonic_embedding_xyz = HarmonicEmbedding(cfg.n_harmonic_functions_xyz)
        # harmonic embedding of the ray directions
        ...
        self.intermediate_linear = torch.nn.Linear(cfg.n_hidden_neurons_xyz, cfg.n_hidden_neurons_xyz)
    ...
    def get_densities(self, features):
        """
        Convert feature to raw_densities with self.density_layer
        """
    ...
    def get_colors(self, features: torch.Tensor, rays_directions: torch.Tensor):
        """
        Takes per-point features and evaluates the color 
        in order to attach to each point its RGB color
        """
    ...
    def get_densities_and_colors(self, features, ray_bundle: RayBundle) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        The 2nd part of the forward pass
        features -- the output of mlp (minibatch x ... x self.n_hidden_neurons_xyz)
        """
    ...
    def forward(self, ray_bundle: RayBundle, **kwargs) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        1st part: Use MLP to map embeddings to a latent feature space
        2nd part: Get density and colors

        Returns:
        rays_densities: opacity of each ray point (minibatch, ..., num_points_per_ray, 1)
        rays_colors: color of each ray point (minibatch, ..., num_points_per_ray, 3)
        """
        # [minibatch x ... x self.n_harmonic_functions*6 + 3]
        ...
        rays_densities, rays_colors = self.get_densities_and_colors(features, ray_bundle)
