...
class VolumeRenderer(torch.nn.Module):
    """
    Integrates color and density along rays according to 
    the equations defined in [Mildenhall et al. 2020]
    """
    ...
    def _compute_weights(self,deltas,rays_density: torch.Tensor,eps: float = 1e-10):
        """
        deltas      ([1, num_rays * num_sample_per_ray, 1]) = ([1, 4194304, 1])
        ray density ([1, num_rays * num_sample_per_ray, 1]) = ([1, 4194304, 1])
        """
    ...
    def _aggregate(self,weights: torch.Tensor,rays_feature: torch.Tensor):
        """
        Aggregate sample points along a ray
        weights      ([1, 4194304, 1])
        rays_feature ([4194304, 3])
        """        
